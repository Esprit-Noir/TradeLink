...
export function DailyGoalWidget({ todayPnl, initialGoal }: Props) {
  ...
  const progress = goal && goal > 0 ? Math.min((todayPnl / goal) * 100, 100) : 0
  const isAchieved = goal !== null && todayPnl >= goal
    ...
    <div className="card" style={{ padding: "1.25rem" }}>
      ...
      {goal !== null && goal > 0 && (
        <>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div style={{ fontSize: "1.25rem", fontWeight: 700, color: todayPnl >= 0 ? "var(--color-profit)" : "var(--color-loss)", fontVariantNumeric: "tabular-nums" }}>
              {todayPnl >= 0 ? "+" : ""}${todayPnl.toFixed(2)}
            </div>
            <div style={{ fontSize: "0.8rem", color: "var(--color-gray-500)" }}>
              {isAchieved ? `🎯 +$${(todayPnl - goal).toFixed(2)} over goal` : isNegative ? `$${(goal - todayPnl).toFixed(2)} to recover + reach goal` : `$${(goal - todayPnl).toFixed(2)} remaining`}
            </div>
          </div>
        </>
      )}
      ...
      {(goal === null || goal === 0) && !editing && (
        <div style={{ textAlign: "center", padding: "0.75rem 0", fontSize: "0.8rem", color: "var(--color-gray-600)" }}>
          Set a daily P&L target to track your progress in real-time.
        </div>
      )}
    </div>
  )
